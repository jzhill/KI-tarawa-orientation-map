...
- `04`: assembles Figure 1 and writes `outputs/orientation_map.pdf` (vector, imagery embedded), `outputs/orientation_map.png` and `outputs/orientation_map_transparent.png` (300 dpi), 5.8 in wide and about 8.1 in tall. Needs the census enumeration areas in `data-raw/KIR_EA_Census2020FINAL.geojson` (Kiribati National Statistics Office; supplied with permission, never committed), the outputs of `02` and `03`, and `reference/tarawa_landmarks.csv`. All figure design (frame, callout positions, colours, titles) is set at the top of the script.
